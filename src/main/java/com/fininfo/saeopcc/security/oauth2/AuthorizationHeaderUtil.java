package com.fininfo.saeopcc.security.oauth2;
...
public class AuthorizationHeaderUtil {
  ...
  private OAuth2AccessTokenResponse refreshTokenClient(OAuth2AuthorizedClient currentClient) {
    ...
    if (currentClient.getRefreshToken() != null) {
      formParameters.add(
          OAuth2ParameterNames.REFRESH_TOKEN, currentClient.getRefreshToken().getTokenValue());
    } else {
      log.error(
          "Unable to refresh token as it's missing for client: {}",
    ...
    try {
      ...
      if (responseEntity.getStatusCode().is2xxSuccessful() && responseEntity.getBody() != null) {
        ...
        // Log the unsuccessful response or null body for debugging.
        log.error(
            "Token refresh failed with status: {} and body: {}",
