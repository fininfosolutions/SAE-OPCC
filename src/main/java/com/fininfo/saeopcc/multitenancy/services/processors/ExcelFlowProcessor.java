package com.fininfo.saeopcc.multitenancy.services.processors;
...
import com.fininfo.saeopcc.multitenancy.domains.flow.BrokerEntityMapping;
import com.fininfo.saeopcc.multitenancy.domains.flow.ExcelFlow;
import com.fininfo.saeopcc.multitenancy.domains.flow.ExcelMessageDescriptionDetails;
import com.fininfo.saeopcc.multitenancy.domains.flow.Flow;
import com.fininfo.saeopcc.multitenancy.domains.flow.FlowConfig;
import com.fininfo.saeopcc.multitenancy.domains.flow.InputExcelFlow;
...
import com.fininfo.saeopcc.multitenancy.services.excelutils.ExcelUtils;
import com.fininfo.saeopcc.multitenancy.services.excelutils.FormatterUtils;
...
public class ExcelFlowProcessor {
  ...
  public List<Notification> inputExcelProcessor(
    ...
    try (Workbook workbook = createWorkbook(file)) {
      if (excelFlowConfig != null
        ...
        loopInputExcelFlow(excelFlowConfig, flowIn, notifications, workbook);
      } else {
        if (excelFlowConfig == null) {
  ...
  public void loopInputExcelFlow(
      ExcelFlow excelFlowConfig, Flow flowIn, List<Notification> notifications, Workbook workbook)
      throws NoSuchFieldException, SecurityException {
    ...
    if (contentBeginLine != null && contentEndLine != null) {
      for (int i = contentBeginLine - 1; i <= contentEndLine; i++) {
        ...
        if (!ExcelUtils.isRowEmpty(row)) {
          rowContents = getRowCellsContent(rowContents, row, inputExcelFlow);
          notifs.add(
  ...
  private String[] getRowCellsContent(
      String[] rowContents, Row row, InputExcelFlow inputExcelFlown) {
